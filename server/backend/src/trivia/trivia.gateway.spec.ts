...
/**
 * The test class, mainly tests the behaviour of the game.
 */
describe("TriviaGateway", () => {
  ...
  it("should update player ready status on ready", () => {
    ...
    expect(gateway.gameManager.game.getPlayers().get(socket.id).isReady).toBe(
      true,
    );
  ...
  it("should give questions to players after the game has started", async () => {
    ...
    // Advance time by 22000ms to make sure we have 2 questions
    for (let i = 0; i < 110; i++) {
      jest.advanceTimersByTime(200);
      await Promise.resolve(); // Allow any pending Promises to resolve
    }
    ...
    expect(
      gateway.gameManager.game.getPlayerById(socket.id).getNbQuestionsInQueue(),
    ).toBe(2); //We also send a question when the game starts
  ...
  it("should kill the player when their stack is full", async () => {
    ...
    expect(gateway.gameManager.game.getPlayerById(socket.id).alive()).toBe(
      false,
    );
